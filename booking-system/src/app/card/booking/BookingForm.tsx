...
function qualificationFields(mode: BookingMode, intent: string) {
  if (mode === "interview") {
    return [
      { key: "role", label: "想應徵的職務", placeholder: "例：台南房仲業務", required: true },
  ...
  if (intent === "buy" || intent === "rent") {
    return [
      { key: "area", label: "希望區域", placeholder: "例：安平、中西區、東區", required: true },
  ...
  if (intent === "sell") {
    return [
      { key: "propertyAddress", label: "物件大約位置", placeholder: "例：安平區某某路，填到路名即可", required: true },
...
export default function BookingForm() {
  ...
  const validate = (): FieldErrors => {
    ...
    if (meetType === "custom" && !customReady) next.meetType = "指定地點必須使用衒志核准後提供的專屬連結。";
  ...
  return (
    <main className={styles.page}>
      <div className={styles.shell}>
        <header className={styles.header}>
          <div className={styles.brand}>{ABIN.name} ‧ {ABIN.company}</div>
          <h1 className={styles.title}>預約與衒志聊聊</h1>
          ...
          {bookingMode && intentKey ? (
            <section className={styles.section}>
              ...
              {approvalState === "approved" && !approvedLocation ? (
                <div className={styles.errorNotice}>核准連結缺少已同意的地點，無法開放指定地點。請聯絡衒志重新產生連結。</div>
              ) : null}
              {approvalState === "invalid" ? (
                <div className={styles.errorNotice}>指定地點核准連結已失效、已使用或被撤銷，請重新聯絡衒志。</div>
              ) : null}
              ...
              {!customReady && approvalState !== "checking" ? (
                <div className={styles.inlineNotice}>
                  需要指定其他地點？請先
                  {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">加衒志私人 LINE</a>
                  {" "}確認。取得專屬連結後，系統會直接帶入核准地點。
                </div>
              ) : null}
              {meetType === "custom" && approvedLocation ? (
                <div className={styles.field}>
                  <span className={styles.label}>衒志已核准的指定地點</span>
                  ...
                  <p className={styles.sectionHint}>此地點已鎖定，若要更換請先與衒志重新確認。</p>
                </div>
              ) : null}
            </section>
          ) : null}
              ...
              <div ref={registerRef("slot")} tabIndex={-1}>
                ...
                {!loadingSlots && !slotError && days.length === 0 ? (
                  <div className={styles.inlineNotice}>
                    ...
                    {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">直接聯絡衒志</a>。
                  </div>
                ) : null}
                {days.length ? (
                  <>
          ...
          {selectedStart && duration && bookingMode && selectedIntent ? (
            <section className={styles.section}>
              ...
              <button className={styles.submit} type="submit" disabled={submitting}>
                {submitting ? "正在送出…" : "送出預約"}
              </button>
              <p className={styles.submitHint}>
                送出後預約即刻成立，不需要再到 Email 點確認。確認信會寄給你，內含改期／取消的管理連結。
              </p>
            </section>
          ) : null}
        </form>
