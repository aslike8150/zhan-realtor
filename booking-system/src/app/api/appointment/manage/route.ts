...
async function loadAppointment(
  ...
  if (!verified) {
    return { error: "連結已失效，請回到最新確認信，或加衒志 LINE 協助處理。", status: 401 };
  }
  let appt = await getAppointment(verified.id);
  if (!appt || appt.email.toLowerCase() !== verified.email.toLowerCase()) {
    return { error: "找不到這筆預約，請加衒志 LINE 協助處理。", status: 404 };
  }
  if (
...
export async function POST(req: NextRequest) {
  ...
  if (action === "confirm" || action === "confirm_attendance") {
    ...
    try {
      ...
      return NextResponse.json(
        {
          error: "預約已確認，但行事曆與通知尚未排入處理。請稍後重新整理；若持續出現，請加衒志 LINE。",
  ...
  if (action === "cancel") {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "距離預約時間太近，請直接加衒志 LINE 協助取消。" },
        { status: 409 },
      );
    }
  ...
  if (action === "reschedule") {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "距離預約時間太近，請直接加衒志 LINE 協助改期。" },
        { status: 409 },
      );
    }
    ...
    try {
      ...
      if (error instanceof GoogleCalendarUnavailableError) {
        return NextResponse.json(
          {
            error: "目前無法確認衒志的行事曆，為避免重複預約，這次沒有改期。請稍後再試。",
