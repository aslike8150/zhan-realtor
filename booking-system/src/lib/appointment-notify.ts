 ...
 * 新預約成立 → ① email 通知系統擁有者 ② LINE 通知系統擁有者（衒志 admin 群）③ email 確認給客戶。
 *
 * 🔴 寄信安全閘（對齊寄信紅線）：
...
// studio / hq：目前只有一個據點，已從 BookingForm 可選清單移除。
// 保留定義，避免萬一有舊資料帶這兩種 meet_type 時查不到名稱而炸掉。
...
export function buildAppointmentIcs(opts: {
  ...
  return [
    ...
    `SUMMARY:${icsEsc(`與衒志預約：${opts.name}`)}`,
...
<table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:#FFFFFF;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(28,45,58,0.08)">
  <tr><td style="background:linear-gradient(135deg,#FF7403,#C35B07);padding:22px 32px">
    <div style="font-size:13px;color:#FFF0E5;letter-spacing:1px">${esc(OWNER.name)} ‧ ${esc(OWNER.company)}</div>
  ...
  <tr><td style="background:#FFF9F5;padding:20px 32px;border-top:1px solid #F0D2BF">
    <div style="font-size:13px;color:#746158;line-height:1.8">
      ${esc(OWNER.name)}（${esc(OWNER.alias)}）‧ ${esc(OWNER.title)}<br>
      📞 ${esc(OWNER.phone)}　📍 ${esc(OWNER.address)}<br>
      LINE：<a href="${esc(SOCIAL.line)}" style="color:#C35B07">${esc(SOCIAL.line)}</a>
    </div>
  </td></tr>
</table>
...
<table role="presentation" class="container" width="560" cellpadding="0" cellspacing="0" style="width:560px;max-width:560px;background:#FFFFFF;border:1px solid #E3EAEE;border-radius:12px;overflow:hidden">
  <tr><td style="height:4px;background:#FF7403;font-size:0;line-height:0">&nbsp;</td></tr>
  <tr><td class="px" style="padding:22px 30px 4px">
    <div style="font-size:13px;color:#746158">${esc(OWNER.name)} · ${esc(OWNER.company)}</div>
    <div style="font-size:20px;font-weight:700;color:#3B261B;margin-top:4px">${esc(opts.title)}</div>
  </td></tr>
  <tr><td class="px" style="padding:8px 30px 26px">${opts.bodyHtml}</td></tr>
  <tr><td style="padding:16px 30px;border-top:1px solid #F7E5D8">
    <div style="font-size:13px;color:#746158;line-height:1.7">${esc(OWNER.name)}（${esc(OWNER.alias)}）· ${esc(OWNER.company)}　電話 ${esc(OWNER.phone)}</div>
  </td></tr>
</table>
</td></tr></table></body></html>`;
}
...
export async function sendCustomerAppointmentConfirmation(a: NotifyInput): Promise<boolean> {
  ...
  const custBody = `
    <p style="font-size:16px;line-height:1.7;margin:0 0 16px">${esc(a.name)} ${honor}你好,衒志已經收到你的預約,時段幫你安排好了。</p>
    ...
    <p style="text-align:center;font-size:13px;color:#7A8896;line-height:2;margin:6px 0 0">
      <a href="${esc(manageUrl)}" style="color:#1F6F83;text-decoration:none">改期 / 取消</a>${mapUrl ? ` · <a href="${esc(mapUrl)}" style="color:#1F6F83;text-decoration:none">地圖導航</a>` : ""} · <a href="${esc(ABIN_PRIVATE_LINE_URL)}" style="color:#1F6F83;text-decoration:none">加衒志 LINE</a>
    </p>
    <p style="font-size:14px;color:#7A8896;line-height:1.7;margin:18px 0 0">有任何狀況,直接回覆這封信或加衒志 LINE 都可以。</p>
  `;
  return sendAppointmentEmailAndRecord({
    appointmentId: a.id,
    purpose: "customer_confirmation",
    to: a.email,
    subject: `你的預約已確認 · 衒志（詹衒志）`,
    html: transactionalEmailLayout({
      title: "預約已確認",
      preheader: `${esc(a.name)} ${honor}你好,衒志已收到你的預約 · ${slotTw}`,
      bodyHtml: custBody,
    }),
  });
}
...
export async function sendCustomerAppointmentChangeEmail(
  ...
  const body = `
    ...
    <p style="font-size:14px;color:#7A8896;line-height:1.7;margin:18px 0 0">
      ${isCancel ? "如果還需要重新預約,可以回信或直接加衒志 LINE。" : "確認信與行事曆請以這封更新後的資訊為準。"}
    </p>
  `;
  return sendAppointmentEmailAndRecord({
    appointmentId: a.id,
    ...
    to: a.email,
    subject: isCancel ? `預約已取消 · 衒志（詹衒志）` : `預約已改期 · 衒志（詹衒志）`,
    html: transactionalEmailLayout({
      title,
...
export async function sendCustomerAppointmentReminder(a: NotifyInput): Promise<boolean> {
  ...
  const slotTw = formatSlotRangeTw(a.slotAt, a.slotEndAt);
  const body = `
    <p style="font-size:16px;line-height:1.7;margin:0 0 16px">${esc(a.name)} ${GENDER_HONOR[a.gender] || ""}你好,提醒你明天有和衒志的預約。</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:15px;line-height:1.9;background:#F7FAFB;border-radius:10px">
      <tr><td style="padding:14px 16px">
        <div>📅 時段　<b>${esc(slotTw)}</b></div>
        <div style="margin-top:4px">📍 方式　<b>${esc(meetTypeLabel(a.meetType))}</b></div>
    ...
    <p style="font-size:14px;color:#7A8896;line-height:1.7;margin:18px 0 0">若需要改期或取消,請直接回信或加衒志 LINE。</p>
  `;
  return sendAppointmentEmailAndRecord({
    appointmentId: a.id,
    purpose: "customer_reminder",
    to: a.email,
    subject: `明天預約提醒 · 衒志（詹衒志）`,
    html: transactionalEmailLayout({
      title: "明天預約提醒",
...
export async function notifyNewAppointment(
  ...
  const locEmailHtml = loc
    ...
    : "";
  // 給 LINE 用的地點文字。固定地點（公司面談/分公司/龍騰建設工作室）衒志本來就知道在哪，
  ...
  // ② LINE 給系統擁有者（衒志 admin 群,自動 mint token）
  const lineText =
    `🔔 新預約\n` +
    `👤 ${a.name} ${honor}\n` +
    `📞 ${a.phone}\n` +
