 ...
 */
// owner.ts 是純常數物件（無 server-only 相依），client 端可安全 import。
...
export const INTENTS = [
  { key: "buy", label: "買房／置產", emoji: "🏠", placeholder: "例:想找安平三房、預算 1,200 萬、自住或置產" },
  { key: "sell", label: "賣房", emoji: "🏷️", placeholder: "例:安平透天想賣、換屋週轉、希望盡快出售" },
  { key: "rent", label: "租賃", emoji: "🔑", placeholder: "例:想租 or 出租、想要的區域、預算、何時要" },
  { key: "legal", label: "稅務／法律諮詢", emoji: "⚖️", placeholder: "例:房地合一稅試算、繼承過戶、產權問題、貸款疑問" },
  { key: "interview", label: "面試", emoji: "🧑‍💼", placeholder: "例:應徵職位、方便聯絡的時間、想先了解的事", interviewOnly: true },
  { key: "other", label: "其他", emoji: "💬", placeholder: "例:資產配置規劃、簡易裝潢建議，或其他想找衒志聊的事" },
] as const;
...
export const MEET_TYPES = [
  { key: "office", label: "公司面談", emoji: "🏢", desc: `${OWNER.company}・${OWNER.address}` },
  // hq / studio：詹衒志目前只有一個據點，已從 BookingForm 的可選清單移除（保留定義避免舊資料查不到名稱）
  { key: "hq", label: "分公司", emoji: "🏛️", desc: "（未啟用）" },
  { key: "studio", label: "工作室", emoji: "🐻", desc: "（未啟用）" },
  { key: "phone", label: "電話聯繫", emoji: "📞", desc: "衒志主動來電" },
...
// 2026-08-12：拿掉「合作 / 拍片洽談」（詹衒志不接業配／課程合作）。
// 相關的 COLLABORATION_INTENTS 與 resolveCollaborationIntent 保留在下方，
// 因為舊資料的 qualification_json 可能還帶著這些欄位，讀取時需要解析。
